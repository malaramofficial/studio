...
const EvaluateWrittenExamOutputSchema = z.object({
  ...
  improvementAdvice: z.string().describe('The advice for improving the performance.'),
  weakAreas: z.array(z.string()).describe('The weak areas that need improvement.'),
});
...
const evaluateWrittenExamPrompt = ai.definePrompt({
  ...
  prompt: `You are an expert teacher evaluating student answers for an exam.
...
Provide the output in JSON format.`, // Enforce JSON output
});
