 ...
 * This is V2, designed to be more robust by enforcing a strict JSON output.
 *
 * - generateWrittenExamV2 - A function that handles the exam generation process.
 * - GenerateWrittenExamInputV2 - The input type for the generateWrittenExamV2 function.
 * - GenerateWrittenExamOutputV2 - The return type for the generateWrittenExamV2 function.
 */

import {ai} from '@/ai/genkit';
import {z} from 'genkit';

const GenerateWrittenExamInputSchemaV2 = z.object({
  subject: z.string().describe('The subject of the exam.'),
  // Chapters are optional, allowing for a general subject-based exam.
...
export type GenerateWrittenExamInputV2 = z.infer<typeof GenerateWrittenExamInputSchemaV2>;

// Define the structured JSON output schema as per the system override.
...
const GenerateWrittenExamOutputSchemaV2 = z.object({
  status: z.literal('success').describe('Indicates that the operation was successful.'),
  exam: ExamSchema,
});
export type GenerateWrittenExamOutputV2 = z.infer<typeof GenerateWrittenExamOutputSchemaV2>;

export async function generateWrittenExamV2(input: GenerateWrittenExamInputV2): Promise<GenerateWrittenExamOutputV2> {
  return generateWrittenExamFlowV2(input);
}

const prompt = ai.definePrompt({
  name: 'generateWrittenExamPromptV2',
  input: {schema: GenerateWrittenExamInputSchemaV2},
  output: {schema: GenerateWrittenExamOutputSchemaV2},
...
const generateWrittenExamFlowV2 = ai.defineFlow(
  {
    name: 'generateWrittenExamFlowV2',
    inputSchema: GenerateWrittenExamInputSchemaV2,
    outputSchema: GenerateWrittenExamOutputSchemaV2,
  },
  async input => {
